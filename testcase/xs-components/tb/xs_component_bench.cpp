...
void drive_gsim(GSIM_CLASS &dut, const Inputs &in)
{
    dut.set_reset(0);
...
Outputs sample_gsim(GSIM_CLASS &dut)
{
    return Outputs{
        dut.get_io$$out0(),
        dut.get_io$$out1(),
...
Outputs eval_gsim(GSIM_CLASS &dut, const Inputs &in)
{
    drive_gsim(dut, in);
    dut.set_clock(0);
    dut.step();
...
void drive_grhsim(GRHSIM_CLASS &dut, const Inputs &in)
{
    dut.reset = false;
    dut.io_in0 = in.in0;
    dut.io_in1 = in.in1;
...
Outputs sample_grhsim(const GRHSIM_CLASS &dut)
{
    return Outputs{dut.io_out0, dut.io_out1, dut.io_out2, dut.io_out3, dut.io_flags, dut.io_checksum};
}

Outputs eval_grhsim(GRHSIM_CLASS &dut, const Inputs &in)
{
    drive_grhsim(dut, in);
    dut.clock = false;
    dut.eval();
...
bool same(const Outputs &lhs, const Outputs &rhs)
{
