...
// - testcase/xiangshan/src/main/scala/xiangshan/backend/VecExcpDataMergeModule.scala
// This file is intentionally self-contained: no local xs-components helper,
// base class, or shared case template is referenced.
class XsReal072BackendVecexcpdatamergemoduleLarge extends Module {
  ...
  private def fold(values: Seq[UInt]): UInt = {
    ...
    for ((value, idx) <- values.tail.zipWithIndex) {
      acc = mix64(acc, value, idx + 1 + 503)
    }
    acc
  }

  private val entries = 80
  private val lanes = 4
  private val updateWays = 5
  private val tapCount = 20
  private val salt = 503
