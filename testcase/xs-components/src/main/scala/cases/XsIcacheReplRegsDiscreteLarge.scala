...
// Source-level distributed-register form of XsIcacheReplRegsLarge. The state is
// written as independent Regs, then reassembled with VecInit for dynamic reads.
class XsIcacheReplRegsDiscreteLarge extends XsComponentModule {
  ...
  for (port <- 0 until ports) {
    readValues(port) := nextStates(readSetBase + (port * 11).U)
  }
