...
// - testcase/xiangshan/coupledL2/src/main/scala/coupledL2/prefetch/TemporalPrefetch.scala
// This file is intentionally self-contained: no local xs-components helper,
// base class, or shared case template is referenced.
class XsReal090PrefetchTemporalprefetchLarge extends Module {
  ...
  private def fold(values: Seq[UInt]): UInt = {
    ...
    for ((value, idx) <- values.tail.zipWithIndex) {
      acc = mix64(acc, value, idx + 1 + 103)
    }
    acc
  }

  private val entries = 48
  private val lanes = 8
  private val updateWays = 3
  private val tapCount = 12
  private val salt = 103
