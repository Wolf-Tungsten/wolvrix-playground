...
- t2/s08（A0032，t2 末步，同日校准型）：c1 t2/main 现行配置（摘除 hotness）
  ...
  c2 e00019 配置同日重测 e00050 中位 **261.1s**（CV 1.22%，同日较 c1 +2.52%），
  但与 08-19 同配置对的 -0.13% 矛盾——affinity+消零运行时效应 ≤ 布局/漂移
  噪声底，仍不可裁；e00019 的 t2 best 地位与 affinity 收益均不作 restart 依据。
  编译杠杆再复证：无消零 emu_build 854.3s vs 287.4s（-66%）。两候选 17/17
  ctest、3 rep difftest 全过；c1 入 t2/main，t2 best 名义 254.533s（较 AM y0
  -6.80%，三轨迹最弱）
- t1/s08（A0031，t1 末步）：c1 新旋钮 `--dead-narrow-storage-elision` 消除
  350,989 个从未被引用的窄成员（窄区 -28.0%，~2.68MB），e00047 中位
  **219.0s**，较同日安慰剂 **-1.36%**（越 1% 假设门，CV 0.94%），已入
  t1/main——死态消除族第三层，收益与体积同向缩放（122MB→-2.78% /
  2.68MB→-1.36%），状态瘦身轴在 t1 收敛关闭；c2 同日安慰剂（t1 tip 原样
  重测）e00048 中位 **222.0s**，较约 4 小时前的 e00041 仅 -0.91%，给出新
  漂移刻度：**同夜 <1%、跨日 ~2.6%**（restart 定 y0 校准 t1 侧锚点已备，
  t0 侧尚缺）。两候选 17/17 ctest、3 rep difftest 全过，compile_s
  530.0/559.8s；t1 best = 218.976s（较 AM y0 -19.82%，gsim 比 8.87x）
...
- 第 7 轮 round-summary（A0029）：run best 降至 **194.792s**（e00040，较 AM y0
  **-28.67%**，gsim 比 **7.89x**，绝对差距关闭 31.52%）；本轮满足 >=3% 干净
  ...
  至预算耗尽。**预算硬约束：余 4 eval < 走满 s08 所需 6，r001 将在预算耗尽处
  提前收口转 run-summary**。C++ 适配层调用边界轴跨轨迹独立复证（t1 e00018
  -9.69% x t0 e00040 -10.02%）收敛为最强机制族；本轮关闭轴：commit gating
  细化（7 变体动态证据全关）、热度加权布局、divmod 内联（基线依赖反转——
  弱正旋钮旧读数不可继承）；布局类收益上限收敛至 ~1-2%。s08 建议：t0 优先、
  t2 做同日校准型候选为 restart 定 y0 留证据
...
- 第 6 轮 round-summary（A0025）：**本轮三轨迹均无干净 >=3% 收益**（t0 名义
  ...
  （需 12）。restart 备选 y0 = t0 扫描剪枝+宽态首触+concat 内联 x t1 窄标量
  helper/常量内联+常量存储消除（机制正交），定 y0 前须同日重测校准（跨日漂移
  ~2.6% > 协议 CV）；本轮关闭轴：宽 helper 内联、宽常量运行时、窄态首触布局、
  commit 读侧快照、atom fold 旋钮。用户无指示时下一 action = t0/s07
