...
- **r004/t0/s02 post-change recon 已完成，幂次索引热池动态 cycles 下降 94.49%**
  ...
  **71.1651%/28.8082%**，总 block cycles 较 t0/s00 e00085 同口径下降 **6.74%**。
  b9029/b9041/b9047/b9062/b9063/b9089 六个 512-depth memory-read 块在执行次数
  不变时由 27.064B 降至 1.492B cycles（**-94.49%**），动态权重由 3.087% 降至
  0.182%，完成幂次索引机制闭环并关闭同池细化。当前剩余主峰为 b93159/b93141
  commit 池 **9.076%**，b69157/b69158/b69159 宽 mux 池 **1.683%**。预算仍为
  **26/48**；下一 action = `r004/t0/s03 step`（K=2），本 action 未启动它。

- **r004 第 2 轮 round-summary 已完成，幂次索引跨五轨迹收敛，宽 mux 是唯一确认二阶增量**
  （2026-08-24，[A0105](actions/A0105_round-summary_r004第2轮跨轨迹与基线重锚_20260824.md)）：
  s02 winner 为 t0/e00100 **172.832s**、t1/e00102 **171.233s**、t2/e00104
  **169.577s**、t3/e00106 **166.947s**、t4/e00108 **189.263s**、t5/e00110
  **173.058s**。幂次 memory-read 索引在四条迁移轨迹较父节点稳定改善 8.95%-10.27%；
  e00106 宽 mux 组合较 e00093 再改善 **3.24%**，成为唯一确认二阶增量。round 2
  retime 得 AM/e00085 **194.019s**（相对冻结 +0.32%）、gsim/e00086 **24.226s**
  （+6.63%，持续漂移）；冻结裁决口径不变。预算 **26/48**，下一 action =
  `r004/t0/e00100 recon`，本 action 未启动它。
...
- **r004/t0/s02 已完成，e00093 幂次索引机制成功迁移到 t0**
  ...
  回退 2.30%，CV 0.92%），干净否决两个弱信号可加；e00100 迁移 e00093 的
  `--memory-read-pow2-index` = **172.832s**（较 e00088 改善 **8.95%**，CV 1.55%），
  两项均以完整 14 开关表型通过 17/17 ctest 与三次 `73580/49996` difftest，单簇且
  非 noisy。e00100 以 `migration_source=e00093`、outcome=`win` 入 t0/main，但与
  e00093 仅差 0.18%，只能确认幂次索引可迁移，不能确认 division 有可加增量。预算
  **16/48**，全局 best 仍为 t3/e00093 **172.530s**；下一 action =
  `r004/t1/s02 step`（K=2），本 action 未启动它。
...
- **r004/t5/s00 recon 已完成，e00085 的第六条初始轨迹动态证据已刷新**
  ...
  compute/commit = **73.1234%/26.8525%**，top-50 = **39.978%**。
  b93159/b93141 commit 主峰合计 **8.421%**，b90656/b90657 system-task compute
  双峰合计 **4.693%**，b83835/b93085 合计 **2.325%**。画像不占评估预算，当前
  仍为 **12/48**；下一 action = `r004/t5/s01 step`（K=2），本 action 未启动它。
...
- **r004/t1/s00 recon 已完成，e00085 的第二条初始轨迹动态证据已刷新**（2026-08-23，
  ...
  compute **73.1976%**、commit **26.7794%**。top-50 块占总 cycles **39.714%**，
  b93159/b93141 commit 主峰合计 **8.495%**，b90656/b90657 task compute 双峰合计
  **4.629%**，b83835/b93085 合计 **2.354%**。画像不占评估预算，当前仍为 4/48；
  下一 action = `r004/t1/s01 step`（K=2），本 action 未启动它。
...
- **r004/t0/s00 recon 已完成，e00085 的动态热点权重已刷新**（2026-08-23，
  ...
  runtime-profile 以生产 12 开关表型重 emit，`73580/49996` 金标精确通过；阶段分解为
  compute **73.4435%**、commit **26.5324%**。top-50 块占总 cycles **39.784%**，
  b93159/b93141 commit 主峰合计 **8.620%**，b90656/b90657 task 守卫残余合计
  **4.683%**，b83835/b93085 比较/归约池合计 **2.391%**。画像不占评估预算，当前仍
  为 2/48；下一 action = `r004/t0/s01 step`（K=2），本 action 未启动它。

- **r004 已初始化，正式 gsim 恢复并完成本机串行双基线**（2026-08-23，
  [A0085](actions/A0085_run-init_r004正式gsim恢复与双基线_20260823.md)）：标准未打平
  emu 已生成到 `build/xs/gsim/gsim-compile/emu`（SHA-256 `73e62bd...a4418`，
  84,643 supernodes / 645,854 DAG edges）。r004 以 r003/e00057、C=6/L=4/K=2
  启动；e00085 AM y0 = **193.403s**，e00086 gsim target = **22.720s**，均为绑核
  串行 3 rep、单簇、非 noisy 且 difftest 通过，当前差距 **8.512x**。A0084 的迁移
  前置阻塞已解除；下一 action = t0 recon，本 action 未启动它。
