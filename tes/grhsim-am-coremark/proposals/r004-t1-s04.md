# Proposal r004/t1/s04（Φ 自动生成）

- task: grhsim-am-coremark  run: r004  trajectory: t1  step: 4/4
- 轨迹主线分支: `tes/r004/t1/main`  best: {"eval_id": "e00113", "score": -166014}
...
### e00113（step 3, score -166014, host 中位 166014 ms）
- 假设: From e00102, post-change recon shows b69157/b69158/b69159 still carry 1.697% of block cycles; migrating confirmed e00106 ArrayBroadcast-to-ArrayMux chain fusion should retain host-call guard caching and power-of-two indexing while reducing this independent pool and improving Host by at least 1% versus e00102.
- 洞察: e00113 passed 17/17 ctest and three 73580/49996 difftest reps; 166.014s unimodal non-noisy Host improves 3.05% versus e00102 and crosses the r004 confirmation band, confirming wide mux fusion transfers additively onto host-call guard caching plus power-of-two indexing.
- 分支 `tes/r004/t1/s03-c1`  commit `2768258e7ab0`  结果 `build/tes/grhsim-am-coremark/evals/e00113/result.json`

### e00089（step 1, score -190827, host 中位 190827 ms）
- 假设: e00085 的 b90656/b90657 合计占动态块 cycles 4.629%，相邻 fwrite/DPI 站共享 fire/event；每 chunk 只计算一次完整 guard 应消除 outlined 调用间的重复成员读取并带来可测池级收益
- 洞察: e00089 全门通过且命中 6314 个共享 guard 组；Host 190.827s、单簇 CV 1.70%，较 e00085 名义改善 1.33%，弱正但未越 r004 的 3% 确认门。
- 分支 `tes/r004/t1/s01-c1`  commit `784a466adeec`  结果 `build/tes/grhsim-am-coremark/evals/e00089/result.json`


## 本轨迹已评估但未中选的变体（避免原样重试；可在其基础上改机制）

- e00090 s1c2: score -191508（191508 ms）— e00085 的 b83835/b93085 合计占动态块 cycles 2.354%，其中密集 scalar concat_value 前缀链；按最终位位置装配 <=64-bit Concat 可去掉串行前缀 mask/shift 并带来可测收益
- e00101 s2c1: score -189609（189609 ms）— Starting from e00089, whose adjacent host-call guard cache improved Host by 1.33% but remained inside the noise band, e00085 recon shows blocks 90656/90657 still represent 4.629% of cycles; extending reuse across intervening instructions that do not write guard/event operands should increase production cache hits and improve Host by at least 0.75% versus e00089 without changing host-call state semantics.

## 失败模式摘要（本轨迹）

- e00114 s3c2: **ctest_fail** — From e00102, post-change recon shows b93159/b93141 still carry 9.194% of block cycles while commit time is unchanged; deferring oversized commit scratch clearing until the event gate opens should avoid zeroing large arrays on quiet rounds, reduce this pool by at least 15%, and improve Host by at least 1% versus e00102.（日志见 `build/tes/grhsim-am-coremark/evals/e00114/result.json`）

## 本轨迹 recon 证据（动态病灶数据）

- 最新 recon: `build/tes/grhsim-am-coremark/recon/r004-t1-s02/report.md`（基于 eval e00102）
...
|---|---|---|---|---|---|---|
| e00113 | -166014 | 1.000 | 1.00 | 0 | 3.236 | * |
| e00102 | -171233 | 0.900 | 0.67 | 1 | 1.645 |  |
| e00089 | -190827 | 0.810 | 0.33 | 1 | 1.183 | * |
