# Proposal r004/t4/s04（Φ 自动生成）

- task: grhsim-am-coremark  run: r004  trajectory: t4  step: 4/4
- 轨迹主线分支: `tes/r004/t4/main`  best: {"eval_id": "e00119", "score": -173624}
...
### e00119（step 3, score -173624, host 中位 173624 ms）
- 假设: Phi e00108 -> post-change t4 recon shows six 512-depth memory-read blocks carrying 3.101% of block cycles while this tip lacks the confirmed specialization -> migrate e00093's exact 9-bit power-of-two indexing to remove 3,072 index_words calls and unreachable bounds branches -> expect Host time to improve by at least 5%, otherwise the mechanism is not additive with sys-task predicate runs
- 洞察: e00119=173.624s，较 e00108 改善 8.26%；17/17 ctest 与三次 difftest 全过，单簇 CV 1.30%、非 noisy；幂次索引与 predicate-run 可加并越过 3% 确认带。
- 分支 `tes/r004/t4/s03-c1`  commit `099335af01bb`  结果 `build/tes/grhsim-am-coremark/evals/e00119/result.json`

### e00085（step 0, score -193403, host 中位 193403 ms）
- 假设: am baseline
- 洞察: r004 y0=r003/e00057/1563c3d8；17/17 ctest 与三次串行 difftest 全过，cores 12/13/14 为 193403/188958/194839 ms，单簇中位 193403 ms，CV 1.59%，非 noisy，compile_s=645.3s。
- 分支 `None`  commit `1563c3d837fc`  结果 `build/tes/grhsim-am-coremark/evals/e00085/result.json`


## 本轨迹已评估但未中选的变体（避免原样重试；可在其基础上改机制）

- e00107 s2c1: score -189851（189851 ms）— Phi e00085 and t4/s01 c1 ctest feedback -> recon b93159/b93141 consume 8.646% of block cycles while the prior fixture incorrectly required a detector-group array absent from this model -> correct that fixture and defer chunked commit scratch clearing until the event gate opens -> expect the pool to fall at least 15% and Host time at least 1%, otherwise falsified
- e00120 s3c2: score -191047（191047 ms）— Phi e00108 -> post-change t4 recon shows b83835/b93085 still consume 2.239% of block cycles, and b83835 contains 1,920 generated replicate-chain lines including repeated 1-bit-by-8 concat_value sequences -> multiply-pack narrow Replicate with a compile-time spread constant -> expect the pool to fall at least 15% and Host time at least 0.5%, otherwise falsified

## 失败模式摘要（本轨迹）

- e00095 s1c1: **ctest_fail** — Defer chunked commit scratch clearing until the event gate opens; the 8.646% b93159/b93141 pool should fall at least 15% and Host time at least 1%.（日志见 `build/tes/grhsim-am-coremark/evals/e00095/result.json`）
- e00096 s1c2: **ctest_fail** — Merge adjacent outlined fwrite calls with identical immediate fire predicates into one branch; the 4.923% b90656/b90657 pool should fall at least 20% and Host time at least 1%.（日志见 `build/tes/grhsim-am-coremark/evals/e00096/result.json`）

## 本轨迹 recon 证据（动态病灶数据）

- 最新 recon: `build/tes/grhsim-am-coremark/recon/r004-t4-s02/report.md`（基于 eval e00108）
...
|---|---|---|---|---|---|---|
| e00119 | -173624 | 1.000 | 1.00 | 0 | 3.000 | * |
| e00108 | -189263 | 0.900 | 0.50 | 1 | 1.400 |  |
| e00085 | -193403 | 0.810 | 0.00 | 12 | 0.810 | * |
