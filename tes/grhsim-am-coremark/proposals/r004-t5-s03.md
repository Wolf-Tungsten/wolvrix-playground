# Proposal r004/t5/s03（Φ 自动生成）

- task: grhsim-am-coremark  run: r004  trajectory: t5  step: 3/4
...
### e00110（step 2, score -173058, host 中位 173058 ms）
- 假设: 迁移来源 e00093：e00085 recon 的六个 512-depth memory-read 块占 3.140% 动态块周期，e00093 已确认幂次索引专化较基线改善 10.79%；在 e00097 scratch 后延父节点上叠加同一机制，预期通过全门且 Host 较 e00097 至少下降 5%
- 洞察: e00110 以 e00093 为唯一迁移源，在 e00097 scratch 父节点上通过 17/17 ctest 与三次 difftest；Host 173.058s、CV 0.54%、单簇非 noisy，较 e00097 改善 9.45%，确认幂次索引可与 scratch 后延组合迁移
- 分支 `tes/r004/t5/s02-c2`  commit `e908a7ed44de`  结果 `build/tes/grhsim-am-coremark/evals/e00110/result.json`
...
|---|---|---|---|---|---|---|
| e00110 | -173058 | 1.000 | 1.00 | 0 | 3.000 | * |
| e00097 | -191116 | 0.900 | 0.50 | 1 | 1.400 |  |
| e00085 | -193403 | 0.810 | 0.00 | 11 | 0.810 | * |
