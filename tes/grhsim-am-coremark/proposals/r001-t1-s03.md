# Proposal r001/t1/s03（Φ 自动生成）

- task: grhsim-am-coremark  run: r001  trajectory: t1  step: 3/8
- 轨迹主线分支: `tes/r001/t1/main`  best: {"eval_id": "e00006", "score": -270502}
...
### e00011（step 2, score -289151, host 中位 289151 ms）
- 假设: 激活合并455K静态/4.77G动态条件分支若显著误预测，去分支化(恒写活动字mask&actMask)后Host中位降≥1%；持平则编译器已吸收，子轴关闭
- 洞察: 289.2s(+6.9% vs t1主线270.5s，CV 0.12%，difftest全过)显著回退、假设证伪：条件激活写是承力设计——恒写activeWords_引入~4.8G次额外线程共享store污染活动字cache行(扫描侧每轮重读)，远超分支消除收益；安静组分支实际高度可预测。激活写去分支化轴关闭；compile_s 2154s(emu_build 1234s)预算内
- 分支 `tes/r001/t1/s02-c1`  commit `3bd89cdb9143`  结果 `build/tes/grhsim-am-coremark/evals/e00011/result.json`
...
|---|---|---|---|---|---|---|
| e00006 | -270502 | 1.000 | 1.00 | 1 | 2.000 | * |
| e00001 | -273103 | 0.900 | 0.50 | 3 | 1.150 |  |
| e00011 | -289151 | 0.000 | 0.00 | 0 | 0.000 | * |
