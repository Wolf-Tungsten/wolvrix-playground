# Proposal r002/t0/s04（Φ 自动生成）

- task: grhsim-am-coremark  run: r002  trajectory: t0  step: 4/8
- 轨迹主线分支: `tes/r002/t0/main`  best: {"eval_id": "e00007", "score": -261543}
- 基线 am: score -619019（host 中位 619019 ms，eval e00001）
- 基线 gsim: score -46792（host 中位 46792 ms，eval e00002）

---

## 任务指令（x0）

# TES 任务指令（x0）：grhsim-am xiangshan coremark 50k 性能优化

这是每个 proposal 都会携带的常驻任务指令。它定义目标、约束与裁决口径，不随实验进展改变；如需修改，修改即构成新的实验条件，必须在 `tes/grhsim-am-coremark/state/insights.md` 追加记录。

## 目标

最小化 grhsim-am emu 仿真 xiangshan coremark 50k 周期（`-C 50000`）的 **Host wall time**（3 rep 中位，`taskset` 绑核，串行无干扰），直至**不劣于 gsim 同等负载**（同一 coremark bin、同一 difftest 协议、同一机器、同一测量协议测得的 gsim emu Host 中位）。

- 分数：`score = -median_host_ms`（越高越好）。
- 进度指标：`ratio = am_median / gsim_median`，目标 `ratio <= 1.0`。
- 参考基线在 run-init 时实测并冻结进 `tes/grhsim-am-coremark/runs/<run>/manifest.json`。

## 硬约束（任一违反即判失败候选）

1. **功能门**：difftest 逐位一致——每 rep 进程退出码 0 且 `instrCnt = 73,580`、`cycleCnt = 49,996`（coremark-2-iteration 50k 窗金标）。
2. **回归门**：`ctest -R grhsim` 全绿（emit-cost 系列确立的 17 项）。
3. **测量纪律**：计时 rep 串行、绑核、无构建等并发负载；计数/计时分离（正式计时不开 `EMU_RUNTIME_PROFILE` / `EMU_AM_BLOCK_EXECS` 等插桩）。
4. **编译预算**：从 cmake 到 emu 二进制就绪的累计墙钟 ≤ **40 min**，超预算即判 `compile_timeout` 失败，不进入计时——生成代码体积/单 TU 复杂度也是成本（NO0007 曾单 TU >40min），病态膨胀的候选没有资格谈运行时。

## 工作面

候选解 = wolvrix 仓库的一个 commit（在 tes 候选分支上）+ 可选的 emit 参数覆盖（`emit_args`，如 `--blocks-per-source`、`--dp-coarsen-*`、`--max-atoms-per-block` 等旋钮）。评估输入固定为 wolvrix 自解析 XiangShan SV 的归一化 GRH（post-stats JSON，见 manifest 指纹），保证跨候选可控对比。

## 优化哲学（变更面纪律，r002 起）

1. **GRH IR 冻结**：候选不得改变 GRH IR 的定义与语义（`wolvrix/include/core/grh.h`、`wolvrix/lib/core/grh.cpp` 等 GRH 数据结构与算子语义），也不得以 ingest 侧行为变化作为收益来源——GRH 是跨工具的稳定契约，冻结它以保证候选间可比、可归因。
2. **grhsim AM 可激进改动**：AM IR（lower 之后的块/atom/调度结构）允许**激进改动**——重构 AM IR 结构、变换规则、程序组织方式均合法，不受 r001 既有形态约束。
3. **调度算法等可激进改动**：调度/分区/布局等算法层同样允许**激进改动**——可以推翻现有算法另起炉灶（如新调度器、新分区目标函数、新块组织），不要求与现行实现保持增量关系。
4. **改进尽可能显式化**：优化手段应尽量体现为**显式的 grhsim AM 优化 pass / 算法阶段**——有名字、可独立开关、能在 pipeline 中定位，便于单变量归因与回撤；避免把变换隐式揉进 emit/lower 的边角逻辑里。即使是激进重构，也应落为可指认、可开关、可文档化的显式机制，而非不可分离的整体改写。
5. **突破性、原创性导向**：r002 不满足于 r001 式的 ~1% 级旋钮微调——候选方案应追求**机制层面的突破与原创性**（新模型、新算法、新表示），以数量级或大幅倍率缩小对 gsim 的差距为目标；平庸的参数扫掠不应占用正式评估预算。
6. **emit 规则变更须配文档**：确需改变 emit 规则的候选，应尽可能同步提供或更新文档（`wolvrix/docs/grhsim/`，如 `grhsim-am-pipeline.md`），说明规则变化与动机；文档随候选 commit 一并提交。

## 已知机制背景（来自 pdocs/grh-notepad/emit-cost 等，run-init 时的起点知识）

- Host 465.8s → 324.0s 的修复链：巨 atom 常量折叠（NO0011）、动态基座元素级发射（NO0012）、装配锥窗口化（NO0013）、dynblend 锥塌缩（NO0014）、窄值标量化（NO0016）、宽状态切片内联 + 掩码写展开（NO0018）。
- 当前残余成本画像（NO0018 收口）：跨 chunk uint64 数组槽 store→load 往返、状态 gather（194MB 模型对象上的值散布）、2.87x instr/atom 适配胶；匹配对加权 4.14x、>100x 尾部 13 对 / 1.5G。
- 已证伪方向：分区形状（NO0002）、存储宽度本身（NO0016 Stage A）；已回撤负优化先例：独立 concat 去零换 replace（NO0013 F2）。
- 测量陷阱先例：`-j32` 构建负载污染计时（supernode-align NO0017）、ASLR/PIE 布局效应（NO0338+）、跨构建 id 漂移导致 join 错配（emit-cost NO0017）。


## 评估协议

# 评估协议（grhsim-am-coremark）

Φ 会把本文件原样内联进每个 proposal 的「评估协议」一节。改协议即改实验条件：
run 期间不修改，修改应在 insights.md 追加记录。

- 负载：`coremark-2-iteration.bin`，`-C 50000`（50k 周期窗），difftest 参考
  `riscv64-nemu-interpreter-so`。
- 计时：3 rep 取中位（CV>5% 自动加测至 5 rep），每 rep `taskset` 绑一个独立
  物理核（见 config `eval.rep_cores`，同 socket、非 SMT 兄弟；2026-08-20 起
  rep 批内并行替代逐 rep 串行），评估间严格串行无干扰（全局 LOCK + 每批次
  起跑前 emu 进程守卫）；不开任何 profile 插桩。
- 功能门：每 rep 退出码 0、nemu 在线逐指令核对无 mismatch，且 instrCnt/cycleCnt 落在
  金标窗内（中心 73,584/49,998，容差 ±16/±8——窗口覆盖 gsim 与 am 两种 emu 在 50k
  周期窗停止点上的确定性小差，见 config `eval.golden_tol`）。
- 回归门：`ctest -R grhsim` 全绿。
- 编译预算：cmake→emu 二进制就绪累计 ≤ 2400s，超预算判 `compile_timeout`。
- 流水线：wolvrix 全量构建（Release + ccache）→ ctest → `grhsim-am-lower-json
  <post_stats_json> SimTop --emit <dir> <emit_args>`（post_stats_json 为 wolvrix
  自解析 SV 的冻结归一化产物；emit_args 见 config）
  → difftest emu 构建 → 计时 reps。
- 可调旋钮（候选可覆盖）：`--emit-args` 透传给 lower-json，如 `--blocks-per-source`、
  `--max-atoms-per-block`、`--tree-atom-fold-max-instr`、`--dp-coarsen-*`、
  `--merge-when-min-group`、`--block-chunk-instructions` 等（取值语义见
  `wolvrix/docs/grhsim/grhsim-am-pipeline.md`）。


## Φ 选中的历史节点（本轮 refinement 的出发材料）

### e00007（step 2, score -261543, host 中位 261543 ms）
- 假设: commit 相 32.4% 集中于每 eval 触发的宽站巨块（43 块独占 31.7% 块周期）；assign_words_detect 通用逐词循环未被向量化（反汇编每词 ~12-15 标量指令），4,103 个同宽站点的 memcmp/memcpy 快速路径在 ~97% idle 率下使 Host 中位较 t0 tip 降 >=8%；<3% 证伪
- 洞察: 确认且超假设：261.543s（CV 0.0%，三 rep difftest 全过 73580/49996，compile_s 1863.9s），较 t0 tip 362.869s 名义 -27.9%；计时窗全程 3.70GHz 恒定，批内一致；机制级收益远超双态混杂带（x1.3-1.4）半宽，为 r002 目前最大单步
- 分支 `tes/r002/t0/s02-c1`  commit `ecb4c3f3c6b2`  结果 `build/tes/grhsim-am-coremark/evals/e00007/result.json`

### e00001（step 0, score -619019, host 中位 619019 ms）
- 假设: am baseline
- 洞察: r002 y0=9c0a89db，新机器 clang21.1.8 + rep 绑核并行协议：619.019s（CV~0.0%，cores 12/13/14），对 gsim 13.23x；并行批均匀抬升 ~15%（同批单跑 539.2s 对照）
- 分支 `None`  commit `9c0a89db94a3`  结果 `build/tes/grhsim-am-coremark/evals/e00001/result.json`

### e00012（step 3, score -363444, host 中位 363444 ms）
- 假设: 安慰剂锚点：t0 tip（e00007 代码 + 同 10 旋钮）原样重测，把 ledger best 261.543s 钉进本窗机器态；若读数回到 ~360s 则 e00007 的 -27.9% 为双态快窗抽签（261.543 x 1.39 = 363.5）而非机制收益
- 洞察: 363.444s（CV 0.0%，门全过，compile_s 1858.8s）= 261.543 x 1.389 精确落带：e00007 名义 -27.9% 确认是双态快态抽签，wide-detect-fast-path 机制本身亚分辨中性（recon-t0s03 块 tick 与插桩 Host 持平旁证）；本窗 = 常态/慢态，t0 tip 真值 ~363s
- 分支 `tes/r002/t0/s03-c2`  commit `48916f2d0f9e`  结果 `build/tes/grhsim-am-coremark/evals/e00012/result.json`


## 本轨迹已评估但未中选的变体（避免原样重试；可在其基础上改机制）

- e00004 s1c2: score -452803（452803 ms）— 新输入图宽池死变量占比与 r001 旧图相当（r001：-65.66% words），重实现 dead-wide-storage-elision 后 Host 中位降 >=2% 且 emu_build 显著下降；<1% 证伪
- e00008 s2c2: score -358456（358456 ms）— 守卫双块（b90656/90657，各 ~7000 atom 的 system.task 条件评估，每 eval 触发）占块周期 9.2%，per-atom ~50cyc 由 ~6574 个 enable 变量散射于 ~126 万窄成员中的 cache miss 驱动；守卫条件变量窄存储聚簇（仅换成员声明序）使 Host 中位较 t0 tip 降 >=5%；<2% 证伪
- e00011 s3c1: score -370498（370498 ms）— 守卫池 9.3%（b90656/90657，65.3G ticks，每 eval 触发的 ~7000 atom system.task 条件评估）由事件门控任务 run 的取指/前端流式成本构成；run 级事件门（门=run 内 changedResults_ 槽 OR，门关闭跳过整段）消除 negedge 半数触发的全量重估，Host 中位较同窗安慰剂降 >=4%；<2% 证伪

## 本 step 任务

设计并实现 **2 个互不相同的候选**（机制层面不同，不是同一想法的参数微调；任务支持的可调旋钮见上方任务指令）。每个候选：在各自 worktree 实施 → 提交到各自候选分支 → 任务 evaluator 串行评估 → `tesctl.py record-eval` 登记。全部完成后 `tesctl.py finish-step` 裁决。

## Φ 选择调试表（RPUCG）

| node | score | U | ρ | n | RPUCG | selected |
|---|---|---|---|---|---|---|
| e00007 | -261543 | 1.000 | 1.00 | 1 | 2.118 | * |
| e00003 | -362869 | 0.900 | 0.67 | 1 | 1.645 |  |
| e00012 | -363444 | 0.715 | 0.33 | 0 | 1.460 | * |
| e00001 | -619019 | 0.810 | 0.00 | 4 | 0.810 | * |
