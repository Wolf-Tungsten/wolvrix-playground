# Proposal r004/t3/s04（Φ 自动生成）

- task: grhsim-am-coremark  run: r004  trajectory: t3  step: 4/4
- 轨迹主线分支: `tes/r004/t3/main`  best: {"eval_id": "e00106", "score": -166947}
...
### e00118（step 3, score -168540, host 中位 168540 ms）
- 假设: Starting from e00106, its fresh recon assigns 4.795% of block cycles to b90656/b90657 and generated b90657 code repeatedly emits adjacent fwrite and DPI calls under identical immediate predicates; grouping heterogeneous adjacent host calls under one shared predicate should reduce that pool by at least 15% and improve Host by at least 0.75% versus e00106.
- 洞察: e00118 passed 17/17 ctest and three 73580/49996 difftest reps; production b90656/b90657 contain 6349 shared fwrite+DPI guards, but 168.540s unimodal non-noisy Host regressed 0.95% versus e00106, falsifying shared predicate evaluation as a material residual cost.
- 分支 `tes/r004/t3/s03-c2`  commit `487700bb3b77`  结果 `build/tes/grhsim-am-coremark/evals/e00118/result.json`

### e00093（step 1, score -172530, host 中位 172530 ms）
- 假设: e00085 recon 的六个 512-depth memory-read 块占总块 cycles 3.140%；精确 9-bit 幂次索引直接掩码可删除 3072 个 index_words 与不可达越界分支，预期该池降至少 25%、Host 降至少 0.75%
- 洞察: e00093=172.530s，较 e00085 改善 10.79%，单簇 CV 0.97%、非 noisy，17/17 ctest 与三次 73580/49996 difftest 全过；六块各 512 站全部专化且无残留 index_words，显著越过 3% 确认门。
- 分支 `tes/r004/t3/s01-c1`  commit `3abdf544eb6e`  结果 `build/tes/grhsim-am-coremark/evals/e00093/result.json`


## 本轨迹已评估但未中选的变体（避免原样重试；可在其基础上改机制）

- e00094 s1c2: score -187491（187491 ms）— e00085 recon shows b69157/b69158/b69159 execute 149,997 times and consume 1.567% of block cycles; fusing their adjacent 64-bit ArrayBroadcast-to-ArrayMux chains so only each 352-lane tail is materialized should cut that pool by at least 70% and Host by at least 0.75%
- e00117 s3c1: score -175776（175776 ms）— Starting from e00106, its fresh recon assigns 9.340% of block cycles to b93159/b93141 and generated code allocates 2742/4227 byte-sized chunk-shared commit scratch flags on each activation; packing these flags into 64-bit words should reduce that pool by at least 10% and improve Host by at least 1% versus e00106.

## 失败模式摘要（本轨迹）

- e00105 s2c1: **ctest_fail** — Starting from e00093, six 512-read blocks still carry 3.140% of e00085 recon cycles and materialize 512 live scalar results before a 1024-bit Concat; fusing each sole-use read group into word-sized packed gathers should reduce that pool by at least 20% and improve Host by at least 0.75% versus e00093.（日志见 `build/tes/grhsim-am-coremark/evals/e00105/result.json`）

## 本轨迹 recon 证据（动态病灶数据）

- 最新 recon: `build/tes/grhsim-am-coremark/recon/r004-t3-s02/report.md`（基于 eval e00106）
...
|---|---|---|---|---|---|---|
| e00118 | -168540 | 1.000 | 1.00 | 0 | 3.236 | * |
| e00106 | -166947 | 1.000 | 0.67 | 1 | 1.745 |  |
| e00093 | -172530 | 0.900 | 0.33 | 1 | 1.273 | * |
