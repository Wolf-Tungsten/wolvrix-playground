# Proposal r004/t5/s04（Φ 自动生成）

- task: grhsim-am-coremark  run: r004  trajectory: t5  step: 4/4
- 轨迹主线分支: `tes/r004/t5/main`  best: {"eval_id": "e00110", "score": -173058}
...
### e00121（step 3, score -168198, host 中位 168198 ms）
- 假设: Migration source e00106: fresh e00110 recon assigns 1.701% of block cycles to b69157/b69158/b69159, while e00106 confirmed wide ArrayBroadcast-to-ArrayMux chain fusion as a 3.24% gain on a power-of-two-index base; adding the same orthogonal mechanism to e00110 should preserve its scratch and memory-read gains and improve Host by at least 1%.
- 洞察: e00121 passed 17/17 ctest and three difftest reps; 4 chains/92 steps hit in production, Host 168.198s (CV 0.25%, unimodal), 2.81% better than e00110 but just inside the 3% confirmation band.
- 分支 `tes/r004/t5/s03-c1`  commit `80c4657cdcdc`  结果 `build/tes/grhsim-am-coremark/evals/e00121/result.json`

### e00097（step 1, score -191116, host 中位 191116 ms）
- 假设: e00085 recon 中 b93159/b93141 合计占 8.421% 块周期；将 chunked commit scratch 清零延迟到合并事件门命中后，预期该池下降至少 15%、Host 下降至少 1%
- 洞察: e00097 正确表型全门通过，Host 191.116s、CV 1.27%、unimodal，较 e00085 改善 1.18%；达到 Host 预期下限但仍在 r004 的 3% 确认带内，热点池降幅需后续 recon 验证；此前漏传开关的同 eval-id 测量未登记。
- 分支 `tes/r004/t5/s01-c1`  commit `64f6a5fd7216`  结果 `build/tes/grhsim-am-coremark/evals/e00097/result.json`


## 本轨迹已评估但未中选的变体（避免原样重试；可在其基础上改机制）

- e00109 s2c1: score -189753（189753 ms）— 来源 e00097 与本轨迹 e00098 失败反馈：e00085 recon 中 b83835/b93085 合计占 2.325% 块周期，e00098 因把首 operand 放到低位而 difftest_fail；改为按剩余总宽从高到低直接装配，并保留 e00097 scratch 后延，预期三次 difftest 恢复且该池下降至少 20%、Host 较 e00097 下降至少 0.5%
- e00122 s3c2: score -170708（170708 ms）— Starting from e00110, fresh recon assigns 4.702% of block cycles to b90656/b90657 and generated b90657 contains thousands of consecutive immediate host calls repeating changedResults_[0]; hoisting that common event into one outer branch per long run should reduce b90657 cycles by at least 20% and improve Host by at least 1% while preserving call order.

## 失败模式摘要（本轨迹）

- e00098 s1c2: **difftest_fail** — e00085 recon 中 b83835/b93085 合计占 2.325% 块周期且生成代码含密集 scalar concat_value 前缀链；按最终位位置直接装配预期该池下降至少 20%、Host 下降至少 0.5%（日志见 `build/tes/grhsim-am-coremark/evals/e00098/result.json`）

## 本轨迹 recon 证据（动态病灶数据）

- 最新 recon: `build/tes/grhsim-am-coremark/recon/r004-t5-s02/report.md`（基于 eval e00110）
...
|---|---|---|---|---|---|---|
| e00121 | -168198 | 1.000 | 1.00 | 1 | 2.118 | * |
| e00110 | -173058 | 1.000 | 0.67 | 1 | 1.745 |  |
| e00097 | -191116 | 0.900 | 0.33 | 2 | 1.148 | * |
| e00085 | -193403 | 0.900 | 0.00 | 13 | 0.900 |  |
