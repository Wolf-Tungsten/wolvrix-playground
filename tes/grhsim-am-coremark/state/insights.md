...
## r004 第 2 轮中段重锚与跨轨迹结论（2026-08-24，action A0105）

- round 2 复用既有 emu 严格串行 retime：AM/e00085 = **194.019s**（相对冻结值
  +0.32%、较 round 1 -0.46%，CV 2.68%），gsim/e00086 = **24.226s**（相对冻结值
  +6.63%、较 round 1 +1.88%，CV 2.93%）；两侧均 unimodal、非 noisy。gsim 漂移
  持续超过 3%，冻结 ledger/manifest 口径不变，重锚只作机器水位诊断。
- 幂次 memory-read 索引已在 t0/t1/t2/t5 四条迁移轨迹较各自父节点稳定改善
  **8.95%-10.27%**，连同原始 t3 构成跨五轨迹一阶复现；后续不再用重复迁移占搜索
  席位，应通过 post-change recon 寻找 `index_words` 消除后的替代热点。
- e00106 的宽 mux 链融合是本轮唯一确认的二阶增量：较 e00093 改善 **3.24%**。
  division、guard cache、predicate run、scratch 后延与幂次索引组合后相对 e00093 的
  差异均在 1.71% 以内，不能确认可加性；e00105 packed gather 因 fixture 结构非法而
  未进入性能测量，仍属未测。
- 前两轮已有确认收益，不触发 24 候选零确认收益停止条件。下一轮先对到期 tip 做 recon，
  用新动态权重驱动局部候选；当前 best e00106 = **166.947s**，冻结 gsim 口径
  **7.348x**，预算 **26/48**。

## r004/t0/s03 宽 mux 迁移与 commit 分块（2026-08-24，action A0107）

- e00111 将 e00106 已确认的宽 ArrayBroadcast-to-ArrayMux 链融合迁移到 e00100，生产
  命中相同的 4 链/92 steps，Host **170.564s**，较父改善 **1.31%**，CV 0.75%、
  单簇非 noisy。它达到 1% 下限但未越 3% 确认带，以 outcome=`neutral` 入 t0/main；
  只记弱正复现，不把宽 mux 与 division 的可加性升级为确认结论。
- e00112 把 chunk 粒度 3000 提至 6000 后 Host **174.258s**，较父回退 **0.83%**。
  gate-head 独立边界使 b93159 chunks 仅由 4 降到 3、b93141 由 6 降到 4，并未实现
...
## r004/t4/s04 宽 mux 确认与 lazy commit scratch 弱正（2026-08-25，action A0123）

- **宽 mux 迁移确认（e00131）**：最新 t4 recon 中 b69157/b69158/b69159 占
  **1.563%** block cycles；把 e00106 已确认的 4-chain/92-step 融合叠加到
  predicate-run + pow2-index tip 后，Host **166.837s**，较 e00119 改善 **3.91%**，
  CV 1.39%、单簇非 noisy，越过确认带并以 `migration_source=e00106` 入 t4/main。
  宽 mux 与该基座确认可加，但跨基座增量仍有 1.31%-3.91% 波动，迁移须以动态覆盖
  和完整组合实测裁决。
- **lazy scratch 未确认加性（e00132）**：同一 recon 的 commit 双峰占 **8.569%**，
  但把 e00107 的清零后延机制组合到 e00119 仅得 **172.265s**，较父改善 0.78%，
  未达 1% 假设且在 3% 确认带内。该机制在多个基座上表现为弱正或回退，降级为布局
  敏感次要机制；重开前先动态拆分 commit 内部成本。
