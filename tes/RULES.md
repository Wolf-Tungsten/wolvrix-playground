...
- 评估严格串行：`build/tes/LOCK` flock；计时阶段前检查无其他 `emu` 进程，有则中止。
...
- 功能门不可协商：每 rep 退出码 0 且 instrCnt=73,580 / cycleCnt=49,996，否则候选
  判 `difftest_fail`，无论多快都不得入选。
- 回归门：`ctest -R grhsim` 全绿。既有失败项（transform-comb-lane-pack / repcut）
  与 grhsim 无关，不在门内；新增失败即 `ctest_fail`。

## 2. 状态与记录规则

- `tes/state/ledger.jsonl` **只追加**：评估结果、commit-marker 一经写入不得修改；
  修正以新条目 + 勘误说明形式追加。
- `tes/state/run.json` 只能由 tesctl.py 或 phi.py 写；人/agent 不手改。
- action 笔记：`tes/actions/Axxxx_<类型>_<主题>_<YYYYMMDD>.md`，Axxxx 全局递增、
  不复用；内容 = 本 action 做了什么、各候选结果、裁决、机制分析、对下一 step 的建议。
  追加不覆盖，同 pdocs 惯例。
- proposal、manifest 生成后不修改（快照语义）。
- 每个 action 结束时：更新 `tes/README.md` 状态速览 → `action-done` 登记 →
  在 playground 当前分支提交 tes/（前缀 `tes(<run>):`）。
- 链接一律相对路径；命令示例里的绝对路径保持文本形式（同 pdocs 规则）。

## 3. git 纪律

- wolvrix 分支只按 DESIGN.md §5 的命名模型创建；轨迹主线只用 `branch -f` 移动，
  永不 checkout 到任何 worktree（候选分支才允许 checkout）。
