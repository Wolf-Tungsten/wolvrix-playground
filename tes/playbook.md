...
3. 设计 **K 个机制互异且都具有实质性能假设**的候选。每个先写下来源 Φ 节点、反馈/
   病灶（必须引用 recon 动态权重，静态计数只作辅证）、局部改动和可证伪预期；不得用
   原样重测或安慰剂占用候选席位。**每个候选 commit 必须随附 worktree 根
   `tes-candidate.json`**（`{"hypothesis": str, "emit_args_add": [...], "emit_args_remove": [...]}`，
   无表型变更也要提交 `{}`）；record-eval 对声明与实际 emit_args 硬审计，不符拒登记。
4. 对每个候选（严格串行、一次一个）：
   a. 在其 worktree 实施（旋钮类候选不改代码，把旋钮记进假设与 tes-candidate.json）。
   ...
   d. 登记：`python3 tes/tools/tesctl.py record-eval --result <result.json> --hypothesis "<假设>" --insight "<结果一句话>"`；
      迁移席位（step≥2、引用他轨迹已确认机制、每 step 至多 1 席）追加
      `--migration-source <来源 eval-id>`。
...
2. **round 2 的中段重锚（协议动作）**：先对两个基线 eval 各做一次
   `python3 tes/grhsim-am-coremark/evaluator.py retime --eval-id <基线 eval>`
   （不占预算、不耗新 eval-id、复用既有 emu），新旧裁决对照写进小结；快带水位漂移
   >3% 时追加 insights 并重估 vs target 口径。
3. 写 action 笔记：跨轨迹对比、哪些机制方向在收敛/发散、是否需要用户调整方向。
4. 可复用结论追加进 `tes/<task>/state/insights.md`（失败模式、已证伪方向、新晋病灶）。
5. `python3 tes/tools/tesctl.py round-summary-done --round <m>`，按 goal.md 收口。
