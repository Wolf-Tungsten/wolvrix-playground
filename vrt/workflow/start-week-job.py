...
def cli_command(cli: str, prompt: str) -> list[str]:
    if cli == "kimi":
        # goal 模式：非交互 print 模式支持 "/goal <目标>" 创建目标，
        # 退出码 0=完成 3=blocked 6=paused（非 0 由调度脚本按失败重试处理）。
        base = shlex.split(os.environ.get("VRT_KIMI_CMD", "kimi"))
        return base + ["-p", "/goal " + prompt]
    if cli == "codex":
        # codex 非交互 exec 暂无官方 goal 入口（其 /goal 为 TUI 特性）；
        # 提示词本身即写明完成判据与产出契约，承担目标语义。
