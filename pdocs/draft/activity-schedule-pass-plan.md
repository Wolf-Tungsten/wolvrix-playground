...
3. `supernode` 间按 boundary value 建立的活动传播关系
...
for each cycle:
    ...
    for supernode in topo_order:
        if guard(supernode):
            clear_activity(supernode)
            eval(supernode)
            propagate_activity(supernode)
    commit_state_updates()
```

单次 `eval()` 内只进行一次前向求值。`commit_state_updates()` 写入的新状态从下一次 `eval()` 开始可见，不在本次 `eval()` 内触发第二轮传播。
...
- `propagated_activity_present`：当前 `supernode` 的活动位已被置位；来源可以是前驱 `supernode` 上某个跨边界 value 的变化传播，也可以是 `eval()` 入口对首批节点的激活。`supernode` 被执行后应立即清掉自身活动位，避免在同一次 `eval()` 中重复执行
- `event_domain_set_hit`：当前 `eval()` 命中了该 `supernode` 关联的某个 `event-domain`；空 `event-domain` 视为恒命中
- `head-eval supernode`：供 emit 构建 `eval()` 入口激活映射使用的 `supernode` 标记。首次 `eval()` 可直接以其作为首批起点；后续 `eval()` 只按输入变化和上一次状态提交结果激活其中相关子集；它不参与 guard 公式
...
`is_head_eval_supernode` 由分图结果直接确定，用于标记在一次 `eval()` 入口可能首先被激活的 `supernode`。它只服务于下一次 `eval()` 的入口 seed，不表示状态提交后在同一次 `eval()` 内重新起跑。判定条件为该 `supernode` 直接消费以下来源之一：
