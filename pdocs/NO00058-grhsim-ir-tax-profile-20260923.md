...
- **机制税可直接归因合计 ≈ 总时间 3-5%**（round-2+ 约 1-2%；negedge eval 的
  ...
  不构成 52% 差距的解释。
- **round-2/不动点直线化的收益池实测 ≈1-3%**（round-2+ 直接工作量：
  commit port_eval 3.0%、commit 进入 5.5%、compute 多轮单元 0.37%，
  加每轮活动字扫描固定开销约 1-2%），低于 3% 节点门，与 NO00046 静态估计
  （~1-2%）一致；且 round 2 承载 negedge 域与历史采样收敛，不可删除——
  用户 2026-09-20 放弃两波直线化的决定由本数据坐实，后续节点不要再回头。
