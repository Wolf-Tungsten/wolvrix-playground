...
| [NO0012](NO0012_动态基座逐元素宽链元素级发射_20260813.md) | 动态基座逐元素宽链的元素级发射改写（不管 commit 成本） | 2026-08-13 | **Tier1+2+3 已收口**：chain replay（规划期常量重指 guard-immune + 收益门）+ lane-write fan 合并（others afterAll 放行后 352-member ROB fan 成形、链真消亡）+ reg.write.dynlane 动态 lane 写新 ISA（writebackTime 358 写改写）；difftest 4/4 逐位一致、ctest 17/17；eval 486.1s→402.3s（-17.2%，引擎比 6.2x，compute 单元 1,221.9G→854.5G）；遗留装配激活 +158G 与窄 atom 机制（decode-isMove/VFMA）待立项 |
