...
当前搜索复盘（每 3 节点更新）：七个已接受机制可组合（共享 commit history、commit 边沿快照、compute history 共享、compute guard 提升、状态读变化激活、提交端口变化激活、活动扫描字打包）。activity-word-scan 消除了逐字节稠密扫描这一固定税：evaluator 侧 compute 53.48→44.82 s、commit 31.61→22.13 s（−16.2%/−30.0%），轮次结构不变（2.0105 轮/eval）；实测收益 +22.1% 超出预注册模型（+4..+7%），超出部分归因于消除约 2.4 G 次/运行的冷分支（BTB 容量）而非仅装载开销。当前最佳均值 **68.223 s**，距约 40 s 目标仍差 28.2 s；活动没有陷入低收益微调（前三节点 −40.35%、−12.54%、−22.11%）。已排除方向维持：ready queue/间接 dispatch、frame 清零移除、activity-mask packing、pending 记录去重、从不写状态读者解除播种（静态证伪仅 31 单元）。后续按残余证据探索：44.82 s compute 相位（平坦分布于 2,662 个任务的真实变化工作 + 播种 system/DPI 层，需输入粒度或值级机制）；commit 残余 22.13 s（含 5141/5142 未批处理多引用事件历史采样、memory cell staged 端口、稳定历史扫描）；publication 1.83 s；约 9% 冻结 harness。本节点为上次复盘后第二个节点。
