...
NO00024 收尾更新（距上次定期复盘 1 个节点）：CPU emitter 将不可变标量常量
直接替换到消费者，使 C++ 编译器折叠恒定 enable、mask 和算术表达式；只改变
4639 个 task，evaluator、初始化、runtime 和布局不变。当前最佳正式样本
**58.700 / 58.782 / 57.771 s**，均值 **58.417667 s**、样本标准差 **0.561529 s**；
同期旧版 **60.902 / 60.687 / 59.453 s**，均值 **60.347333 s**，六次全秩分离、
降低 **3.197601%**。第三对新旧均更快，全部保留；没有采用筛选或跨窗口样本。
完整生成/编译 **595.01/229.70 s** 与全部等价门槛通过，距约 40 s 仍差
**18.417667 s**，长期目标未完成。

最终独立相位诊断 Host **57.928 s**，compute **41.433633 s（71.73%）**、commit
**14.684130 s（25.42%）**、publication **1.565766 s**，evals/rounds **100102/201258**。
该诊断不进入收益统计，也不与旧节点相位跨窗口相减。常量的 127 万静态引用没有
等量转化为动态节省，初始 5–20% 幅度预期未获支持，但正收益由规定统计门槛确认。
下一方向应优先考察依赖/条件驱动的 compute 子图执行或 staged memory 访问，先
刷新当前动态覆盖，避免回到已否定的局部结果转发、bool 包装和通知阈值微调。
本节点已完成，未启动下一节点。

NO00025 收尾更新（距 NO00023 定期复盘 2 个节点）：scalar memory cell 先求
当前版本的 masked next，未变化时不拷贝、不入队，仅首次变化创建 pending；
覆盖 33134 memWrite、158 memFill、433 memWriteSeq，只改变 416 个 commit task
和 header。最终正式样本 **57.602 / 57.388 / 57.656 s**，均值 **57.548667 s**、
样本标准差 **0.141737 s**；同期旧版 **58.594 / 58.895 / 58.788 s**，均值
**58.759000 s**，六次全秩分离、降低 **2.059826%**。完整生成/编译
**605.49/232.94 s**，flat GRH 和完整 GrhSIM checkpoint 与 NO00024 逐字节相同。
距约 40 s 尚差 **17.548667 s**，长期目标未完成。

独立相位 Host **57.794 s**，compute **42.329203 s（73.46%）**、commit
**14.443843 s（25.07%）**、publication **0.774273 s**，evals/rounds
**100102/201258**；不混入性能基线，不跨窗口相减作分相位收益证据。
本次先测试 mux 独占链条件执行，全量/直接写回/仅宽值/宽值 chunk 门槛均未
获益，第四版完整六次均值反而慢 0.638088%，全部保留并撤销实现。避免继续
循环这些阈值；下一步优先考察基于变化依赖的跨任务计算消除或内存端口执行，
先刷新动态证据，不把静态 33725 个写操作视为动态 no-op 数量。NO00026 收尾
应完成三节点定期复盘；本节点归档后停止，未启动下一节点。

NO00026 收尾更新（覆盖 NO00024–26）：普通状态共享在现有
`grhsim.canonicalize-compute` 中按完整初始化、写口和私有事件 history 证明逐轮
等价，合并读取并递归清理副本，使 checkpoint states **508487→467657**、operations
**4095805→3988626**。完整 SV→C++/fresh 编译 **657.43/239.56 s**，flat GRH 与
NO00025 逐字节相同；正式六次交替全部 NEMU PASS，新均值 **55.700000 s**、旧均值
**57.176667 s**，降低 **2.582639%**，全秩分离且 U=0、p=0.05。NO00024、NO00025、
NO00026 三节点同期降幅为 **3.197601% / 2.059826% / 2.582639%**，当前最佳距约
40 s 尚差 **15.700000 s**。本节点否定并撤销了独立依赖分组、set-bit 分派、bit
广播、concat 字组装、布尔函数/常量传播、宽常量及两种独占内存提交；后续须先刷新
当前动态热点，再研究依赖驱动或批量执行，不能跨窗口相乘累计收益。本节点已完成，
未启动下一节点。
