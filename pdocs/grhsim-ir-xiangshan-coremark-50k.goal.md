...
截至 2026-09-14，NO00026 已接受，最终性能目标未完成。固定配置：
...
- 当前最佳为 [NO00026 普通状态共享节点](NO00026-grhsim-ir-candidate-boolean-functions-20260914.md)：正式新样本 **55.590 / 55.900 / 55.610 s**，均值 **55.700000 s**、样本标准差 **0.173494 s**；同窗口旧版为 **57.120 / 57.030 / 57.380 s**，均值 **57.176667 s**、样本标准差 **0.181751 s**。均值降低 **2.582639%**，`max(new)=55.900 < min(old)=57.030`，Mann–Whitney U=0、单侧精确 p=0.05，old−new Cohen's d=8.311284；六次均 NEMU PASS。完整 SV→C++ 生成 **657.43 s**、fresh 32-job 编译 **239.56 s**，均低于 1800 s；距约 40 s 仍差 **15.700000 s**。实现子模块 commit `cafaf61`。
- NO00025 [scalar memory staging 节点](NO00025-grhsim-ir-candidate-memory-noop-20260914.md)：正式新样本 **57.602 / 57.388 / 57.656 s**，均值 **57.548667 s**、样本标准差 **0.141737 s**；同窗口连续旧/新交替的对照为 **58.594 / 58.895 / 58.788 s**，均值 **58.759000 s**、样本标准差 **0.152581 s**。均值降低 **2.059826%**，`max(new)=57.656 < min(old)=58.594`，Mann–Whitney U=0、单侧精确 p=0.05，old−new Cohen's d=8.219088；全部六次保留。距约 40 s 仍差 **17.548667 s**。该节点预选止损线 **87.627 s**（1.5 × NO00024 均值，毫秒取整）。
...
最近一次相位诊断属于 NO00025，数据见上。最近一次平坦 profile 属 NO00024，由 NO00025 开始时刷新：Host **59.401 s**、emu 墙钟 **59.44 s**、11884 samples（200 Hz），compute tasks **63.934702%**、commit tasks **20.304611%**、evaluator **3.130259%**、main-other **9.861999%**、external **2.583305%**，compute 热点分散在 **2329** 个 task，独立 `cpu_stage_cell` 占 **3.315382%**。这些采样不代表 NO00026 当前平坦热点，也不构成收益证据。NO00024–26 的同期收益分别为 **3.197601% / 2.059826% / 2.582639%**，当前仍有约 73% compute、25% commit，距约 40 s 尚差 **15.70 s**。NO00026 已否定独立依赖分组、活动字 set-bit 分派、bit 广播、输出字组装、布尔函数/常量传播、宽常量及两种独占内存提交；后续避免重复这些方向，先重新采集动态覆盖，再提出依赖驱动或批量执行机制。不能将跨窗口均值相减或相乘当作累计收益；下一节点仍须重新预选截止、预注册六次交替。
