# NO0055 `merge-reg` Indexed-Bundle-Entry Only CoreMark 50k Snapshot
...
- `merge-reg` pass 只开启 `enable_indexed_bundle_entry_to_wide_register`
...
```bash
make xs_wolf_grhsim_emu RUN_ID=20260502_indexed_bundle_entry_only_fresh \
...
```bash
make run_xs_wolf_grhsim_emu RUN_ID=20260502_indexed_bundle_entry_only_fresh \
...
```text
build/logs/xs/xs_wolf_grhsim_build_20260502_indexed_bundle_entry_only_fresh.log
```

`merge-reg` 日志确认本轮只启用了 `indexed-bundle-entry-to-wide-register`：

```text
merge-reg: merge-reg: graphs=1 candidate_clusters=2196 candidate_members=40317 bundle_pipeline_clusters=0 bundle_pipeline_members=0 indexed_bundle_entry_clusters=9752 indexed_bundle_entry_members=173695 rewritten_clusters=12197 rewritten_members=171959 scalar_to_memory_changed=false strategies=indexed-bundle-entry-to-wide-register
```

关键点：

- `strategies=indexed-bundle-entry-to-wide-register`
- `scalar_to_memory_changed=false`
- `bundle_pipeline_clusters=0`
- `indexed_bundle_entry_clusters=9752`
- `indexed_bundle_entry_members=173695`
- `rewritten_clusters=12197`
- `rewritten_members=171959`

本轮 `activity-schedule` 关键规模：

```text
supernodes=82368 dag_edges=1147424 ops_mean=62.857 ops_median=70.0 ops_p90=73 ops_p99=108 ops_max=1872
```
...
```text
build/logs/xs/xs_wolf_grhsim_20260502_indexed_bundle_entry_only_fresh.log
```
...
- 只启用 `merge-reg` 的 `indexed-bundle-entry-to-wide-register` 策略后，XS `grhsim` 可以完整 emit 并编译出 `emu`。
...
- 当前证据说明 `indexed-bundle-entry-to-wide-register` 单独开启在本轮 50k 口径下可通过；`scalar-to-memory` 单独开启仍是更可疑的运行时语义问题来源。
