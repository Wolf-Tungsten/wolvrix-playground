# NO0056 `merge-reg` OneHot-Indexed-Bank Only CoreMark 50k Snapshot
...
- `merge-reg` pass 只开启 `enable_onehot_indexed_bank_to_wide_register`
...
```bash
make xs_wolf_grhsim_emu RUN_ID=20260502_onehot_indexed_bank_only_fresh \
...
```bash
make run_xs_wolf_grhsim_emu RUN_ID=20260502_onehot_indexed_bank_only_fresh \
...
```text
build/logs/xs/xs_wolf_grhsim_build_20260502_onehot_indexed_bank_only_fresh.log
```

`merge-reg` 日志确认本轮只启用了 `onehot-indexed-bank-to-wide-register`：

```text
merge-reg: merge-reg: graphs=1 candidate_clusters=8397 candidate_members=96175 bundle_pipeline_clusters=0 bundle_pipeline_members=0 indexed_bundle_entry_clusters=0 indexed_bundle_entry_members=0 rewritten_clusters=0 rewritten_members=0 scalar_to_memory_changed=false strategies=onehot-indexed-bank-to-wide-register
```

关键点：

- `strategies=onehot-indexed-bank-to-wide-register`
...
```text
supernodes=77476 dag_edges=1061326 ops_mean=66.861 ops_median=70.0 ops_p90=79 ops_p99=144 ops_max=2231
```

注意：虽然开关已启用，但本轮 XS 图上 `onehot-indexed-bank-to-wide-register` 没有实际 rewrite 命中。
...
```text
build/logs/xs/xs_wolf_grhsim_20260502_onehot_indexed_bank_only_fresh.log
```
...
- 只启用 `merge-reg` 的 `onehot-indexed-bank-to-wide-register` 策略后，XS `grhsim` 可以完整 emit 并编译出 `emu`。
- `coremark` 50k bounded run 可以跑满 `50000` cycle，并以 cycle limit 正常结束。
- 本轮没有在 [`NO0053`](./NO0053_merge_reg_scalar_to_memory_only_coremark_50k_20260502.md) 记录的 `AheadBtbReplacer.sv:179`、`cycleCnt = 8687` 触发 assertion。
- 但本轮 `rewritten_clusters=0`、`rewritten_members=0`，因此该结果只能说明“开启该策略且其它策略关闭”的流程可通过，不能证明 `onehot-indexed-bank-to-wide-register` rewrite 本身在 XS 上已被运行时验证。
