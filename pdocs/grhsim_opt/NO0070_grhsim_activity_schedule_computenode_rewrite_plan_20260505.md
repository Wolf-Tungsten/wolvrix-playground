...
`source op` 是可被复制到 consumer 局部的叶子 op：
...
- source op 被某个 `commitSupernode` 或 `computeNode` 需要时，应复制到该节点内部。
...
- sink op 的直接 source 输入应复制进所在 `commitSupernode`。
- `commitSupernode` 必须在最终调度模型中保留其输入 value 依赖，尤其是 computeSupernode -> commitSupernode 的边；但 commitSupernode 不作为 value producer 产生出边。

### compute op

`compute op` 是除 source / sink / 声明类 op 之外，参与 compute 阶段执行的 op。
...
- 由 sink op 聚类得到。
- 只在 commit 阶段执行。
- 受独立参数 `maxOpInCommitSupernode` 控制大小，单位是 commitSupernode 内的 op 数量。
- 按 event guard 聚类。
...
3. 如果 operand 的 def 是 source op，则为该 sink op 所属 commit cluster 复制一份 source op，并改接 operand。
4. sink op 与复制进来的 source op 一起形成 commit partition 的成员。
5. 按 event key 聚类。
6. 按 `maxOpInCommitSupernode` 切 chunk。

不变量：

- commitSupernode 内允许 sink op 和本地 source clone。
...
- commitSupernode 中 sink op 的非 source 直接输入。
- graph output port value。
- graph inout port 的 `out` / `oe` 分量。

若 root value 由 source op 定义，则不建立跨边界 node，而是在需要处复制 source。
...
5. 遇到 sink op，直接诊断报错；compute root 不应穿越 sink，出现这种边说明前置 commitSupernode / root 收集或 IR 语义已经错误。
6. 遇到普通 compute op：
...
- commitSupernode 中已经复制的 source user 不应算作跨 computeNode consumer。
...
- commitSupernode 的 input root 只作为 compute DAG 的 observable sink 边界，不作为 compute DAG node。
...
- `source_clones_in_commit_supernodes`
...
`value_fanout` 可以包含 computeSupernode 到 commitSupernode 的 consumer 边，用于表达 commitSupernode 对 compute value 的依赖。commitSupernode 不作为普通 value producer 产生出边；commit 后激活 reader 仍通过 state-read mapping 处理。
...
1. source op 不跨 computeNode / commitSupernode 边界；需要处复制。
...
6. 最终 `value_fanout` 不包含 source op result；必须保留 computeSupernode -> computeSupernode 和 computeSupernode -> commitSupernode 边；不包含 commitSupernode 作为 value producer 的出边。
7. mixed sink/output root 不允许形成 quotient cycle。
8. computeNode 反向建树时不允许遇到 sink op；遇到即报错，不做静默截断。
9. `kSystemTask` / `kDpicCall` 按普通 compute op 处理；它们不需要额外 hard-boundary，因为 compute op 本身没有复制语义。
...
- 校准 `state_read_supernodes` 和 `value_fanout`：
  - source result 不应出现在 value fanout。
...
- source boundary values 归零或接近归零，尤其 `kConstant` / `k*ReadPort` 不应再作为 schedule boundary value 主体。
...
- 输出 `compute_nodes` / `common_expr_compute_nodes` / `source_clones_*` 等统计，用于判断结构是否符合预期。
...
- source clone 目前通过旧的 tail absorb 路径完成，命名和职责不匹配。
