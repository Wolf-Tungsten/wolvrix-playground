# NO0058 `merge-reg` Shift-Chain Only CoreMark 50k Snapshot
...
- `merge-reg` pass 只开启 `enable_shift_chain_to_wide_register`
...
```text
build/logs/xs/xs_wolf_grhsim_build_20260502_221011.log
```

`merge-reg` 日志确认本轮只启用了 `shift-chain-to-wide-register`：

```text
merge-reg: merge-reg: graphs=1 candidate_clusters=8397 candidate_members=96175 bundle_pipeline_clusters=0 bundle_pipeline_members=0 indexed_bundle_entry_clusters=0 indexed_bundle_entry_members=0 rewritten_clusters=64 rewritten_members=3565 scalar_to_memory_changed=false strategies=shift-chain-to-wide-register
```

关键点：

- `strategies=shift-chain-to-wide-register`
- `scalar_to_memory_changed=false`
- `bundle_pipeline_clusters=0`
- `indexed_bundle_entry_clusters=0`
- `rewritten_clusters=64`
- `rewritten_members=3565`

本轮 `activity-schedule` 关键规模：

```text
supernodes=77478 dag_edges=1061465 ops_mean=66.798 ops_median=70.0 ops_p90=79 ops_p99=144 ops_max=1972
```
...
```text
build/logs/xs/xs_wolf_grhsim_20260502_223222.log
```
...
- 只启用 `merge-reg` 的 `shift-chain-to-wide-register` 策略后，XS `grhsim` 可以完整 emit 并编译出 `emu`。
- `coremark` 50k bounded run 可以跑满 `50000` cycle，并以 cycle limit 正常结束。
- 本轮实际命中规模为 `rewritten_clusters=64`、`rewritten_members=3565`，比 [`NO0057`](./NO0057_merge_reg_bitset_only_coremark_50k_20260502.md) 的 bitset-only 命中规模更大。
...
- 当前证据说明 `shift-chain-to-wide-register` 单独开启在本轮 50k 口径下可通过；`scalar-to-memory` 单独开启仍是更可疑的运行时语义问题来源。
