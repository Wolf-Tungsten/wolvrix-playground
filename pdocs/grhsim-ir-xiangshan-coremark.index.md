...
| 尺寸上限细化 compute supernode 以降低 op 求值量（dynOps 由激活粒度决定的命题） | NO00006：cap 128→64（unit 1.817×、ops/activation −47.7%），激活数 +60.6%（扇出发布 +51.9%：消费者摊入更多单元，value→consumer-unit 边密度控制激活数），dynOps 弹性 ≈−0.2 仅 −16.01%；boundary 写 −14.2%（同步下降非增长）；instr −6.05% 被 CPI +6.49%（branch-miss 密度 +37%、icache +19%、evaluator 份额 +0.98pp）全数抵消，Host +0.09% 持平 | gsim 低 op 求值量**非**粒度效应：同尺寸（~60 op）激活率 ir 27.4%/cycle 对 gsim ~10.6%（instr 加权估计）仍差 ~2.6×——激活率/扇出精度结构（单 pass 静态序 + 逐 super 精确扇出）才是差距主因；继续细化单调恶化 CPI。后续激活率机制（支撑集纯化锥、输入端口级精度、扇出结构、单 pass 调度）不受本条证伪约束，但立项须论证如何在不增长扇出边密度的前提下降低激活率 |
