...
| 机制类 | 机制层面证据（节点） | 备注 |
|---|---|---|
| 旧归档锚点复现 | 旧分支归档的 GrhSIM-IR 二进制的 emit 分组与 committed 源码任何组合（含 keep-origins 开关、交叉检查点 reemit）的产物均不一致，精确 provenance 不可复原（NO00001） | 已按用户裁定弃用；勿以旧归档值作为目标或比较基准，一切以 NO00001 定标的 77.155 s 基线为准 |
