...
XS_WOLF_GRHSIM_AM_FULL_EVALUATION ?= 0
XS_WOLF_GRHSIM_AM_CHANGED_TRACE ?= 0
XS_WOLF_GRHSIM_AM_RUNTIME_PROFILE ?= 0
...
XS_WOLF_GRHSIM_SIM_DEFINES ?= $(XS_SIM_DEFINES) GSIM
XS_WOLF_GRHSIM_DEFINE_FLAGS := $(foreach d,$(XS_WOLF_GRHSIM_SIM_DEFINES),-D "$(d)")
...
xs_wolf_grhsim_am_emit: $(XS_WOLF_FILELIST_ABS) xs_grhsim_am_tool
	...
	@set -o pipefail; { \
		...
		$(PYTHON) "$(XS_WOLVRIX_GRHSIM_AM_SCRIPT)" \
			...
			--fanout-absorb-budget-mult "$(XS_WOLF_GRHSIM_AM_FANOUT_ABSORB_BUDGET_MULT)" \
			--fanout-absorb-max-consumers "$(XS_WOLF_GRHSIM_AM_FANOUT_ABSORB_MAX_CONSUMERS)" \
			$(if $(filter 1,$(XS_WOLF_GRHSIM_AM_RUNTIME_PROFILE)),--runtime-profile,) \
			$(if $(filter 1,$(XS_WOLF_GRHSIM_AM_FULL_EVALUATION)),--full-evaluation,) \
			$(if $(filter 1,$(XS_WOLF_GRHSIM_AM_CHANGED_TRACE)),--changed-trace,) \
