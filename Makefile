...
XS_WOLF_GRHSIM_POST_REG_TO_MEM_JSON ?= $(XS_GRHSIM_BUILD)/wolvrix_xs_post_reg_to_mem.json
...
XS_WOLF_GRHSIM_RESUME_FROM_PRE_REG_TO_MEM_JSON ?= $(if $(filter 1,$(XS_WOLF_GRHSIM_RESUME_FROM_POST_REG_TO_MEM_JSON)),0,$(if $(wildcard $(XS_WOLF_GRHSIM_PRE_REG_TO_MEM_JSON)),1,0))
...
XS_WOLF_GRHSIM_POST_REG_TO_MEM_JSON_ABS := $(abspath $(XS_WOLF_GRHSIM_POST_REG_TO_MEM_JSON))
...
XS_WOLF_GRHSIM_EMIT_RUNTIME_STATS ?= 0
...
xs_wolf_grhsim_emit: $(XS_WOLF_FILELIST_ABS) $(XS_WOLF_DEPS)
	...
	@mkdir -p "$(dir $(XS_WOLF_GRHSIM_POST_REG_TO_MEM_JSON_ABS))"
	...
	@set -o pipefail; { \
		...
		WOLVRIX_XS_GRHSIM_RESUME_FROM_POST_REG_TO_MEM_JSON="$(XS_WOLF_GRHSIM_RESUME_FROM_POST_REG_TO_MEM_JSON)" \
		WOLVRIX_XS_GRHSIM_POST_REG_TO_MEM_JSON="$(XS_WOLF_GRHSIM_POST_REG_TO_MEM_JSON_ABS)" \
		...
		WOLVRIX_XS_GRHSIM_SCHED_BATCH_MAX_ESTIMATED_LINES="$(XS_WOLF_GRHSIM_SCHED_BATCH_MAX_ESTIMATED_LINES)" \
			...
			WOLVRIX_XS_GRHSIM_EMIT_RUNTIME_STATS="$(XS_WOLF_GRHSIM_EMIT_RUNTIME_STATS)" \
			$(PYTHON) $(XS_WOLVRIX_GRHSIM_SCRIPT) \
			$(XS_WOLF_FILELIST_ABS) \
			$(XS_SIM_TOP) \
...
xs_gsim_emu: xs_gsim_rtl
	...
	@echo "[LOG] Capturing build output to: $(XS_BUILD_LOG_FILE)"
	@printf '' > "$(XS_BUILD_LOG_FILE)"
	@echo "[CMD] NOOP_HOME=$(XS_NOOP_HOME) GSIM_EMIT_RUNTIME_STATS=$(XS_GSIM_EMIT_RUNTIME_STATS) $(MAKE) $(if $(strip $(XS_VM_BUILD_JOBS)),-j $(XS_VM_BUILD_JOBS),) -C $(XS_ROOT)/difftest emu BUILD_DIR=$(XS_GSIM_BUILD_ABS) GEN_CSRC_DIR=$(XS_DIFFTEST_GEN_DIR_ABS) GEN_VSRC_DIR=$(XS_DIFFTEST_GEN_DIR_ABS) RTL_DIR=$(XS_RTL_DIR_ABS) SIM_TOP_V=$(XS_SIM_TOP_V) NUM_CORES=$(XS_NUM_CORES) RTL_SUFFIX=$(XS_RTL_SUFFIX) EMU_THREADS=$(XS_EMU_THREADS) VM_BUILD_JOBS=$(XS_VM_BUILD_JOBS) EMU_RANDOMIZE=0 WITH_CHISELDB=$(XS_WITH_CHISELDB) WITH_CONSTANTIN=$(XS_WITH_CONSTANTIN) GSIM=1 GSIM_BIN=$(XS_GSIM_BIN)" | tee -a "$(XS_BUILD_LOG_FILE)"
		NOOP_HOME=$(XS_NOOP_HOME) GSIM_EMIT_RUNTIME_STATS=$(XS_GSIM_EMIT_RUNTIME_STATS) $(MAKE) $(if $(strip $(XS_VM_BUILD_JOBS)),-j $(XS_VM_BUILD_JOBS),) -C $(XS_ROOT)/difftest emu \
...
xs_diff_clean:
	rm -rf "$(XS_REF_BUILD_ABS)/verilator-compile" \
		...
		"$(XS_WOLF_GRHSIM_EMIT_DIR_ABS)" \
		"$(XS_WOLF_GRHSIM_POST_REG_TO_MEM_JSON_ABS)" \
		"$(XS_WOLF_REPCUT_PACKAGE_DIR_ABS)" \
		"$(XS_REPCUT_EMU_BUILD_ABS)" \
